  ...
  >
    ...
    >
      ...
      <el-table-column
        ...
        v-bind="{ ...column.columnConfig, ...$attrs, ...column }"
        show-overflow-tooltip
      >
        <!-- :index="column.allSort ? typeIndex : 'index'" -->
        <template #header>
          <i
            v-if="column.required && !props.disabled"
            style="color: #f56c6c"
          >
            *
          </i>
          ...
          >
            ...
            <template v-if="column.slotName">
              <slot
                ...
                :item-data="scope.row"
                :idx="scope.$index"
                :disabled="column?.configEdit?.attrs?.disabled"
              ></slot>
            </template>
            ...
            <div
              v-else-if="!column?.hasOwnProperty('configEdit') && !column.slotName"
              class="text"
              :class="column?.columnConfig?.align ? `is-${column.columnConfig.align}` : ''"
            >
...
<script lang="ts" setup>
  ...
  function typeIndex(index: number) {
    const tabIndex =
      index + ((_paginationConfig.value?.pageNum || 1) - 1) * (_paginationConfig.value?.pageSize || 10) + 1
    return tabIndex
  }
  function add() {
